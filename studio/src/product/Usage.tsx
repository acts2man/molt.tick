...
export function UsagePage({jobs,connected}:{jobs:Job[];connected:boolean}){
 ...
 return <>
  <header className="page-title">
   ...
   <h1>Usage, without the guesswork.</h1>
   <p>Owner beta telemetry and a preview of the credit model. No customer subscription or Molt credit wallet is active.</p>
  </header>
  <div className="owner-explainer">
   <h3>API usage is not the same as Molt credits.</h3>
   <p>Your OpenAI account pays for actual model requests. Proposed Molt credits describe a customer's reconstruction scope. They are not OpenAI tokens, Netlify credits, or dollars. Rejected model responses and repairs can still incur provider usage.</p>
  </div>
  ...
  {recorded.length>0&&<section className="panel issues-panel">
   <h2>Reported per-job usage</h2>
   {recorded.map(j=>{const u=j.report?.usage??j.usage;return <div className="wizard-summary" key={j.id}>
    <div><dt>{j.name}</dt><dd>{u.records?.[0]?.model??'Unknown model'} · {u.calls??0} calls / {u.costEstimate?.estimatedUsd!=null?'~$'+u.costEstimate.estimatedUsd.toFixed(2)+' API estimate':'No price estimate'}</dd></div>
  ...
  <a href="/plans" className="text-button">See proposed plans and examples &rarr;</a>
 </>;
}
