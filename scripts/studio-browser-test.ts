...
try{
 for(const width of [1440,768,390]){
  ...
  await ctx.route('**/api/molt/session',route=>route.fulfill({contentType:'application/json',body:JSON.stringify({authenticated:true,authorized:false,claimable:true,connected:false,login:null,email:'owner@example.test',serverReady:true,repository:'acts2man/molt.tick',branch:'main'})}));
