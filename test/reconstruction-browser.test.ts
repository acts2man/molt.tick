...
const html=(route:string)=>`<!doctype html><html><head><meta charset="utf-8"><title>Studio</title><style>${CSS}</style></head><body><header><img src="logo.svg" alt="Studio mark"><a href="/">Studio</a><a href="/about">About</a></header><main><h1>${title(route)}</h1><p>${text(route)}</p><a class="accent" href="/about">Explore our work</a></main><footer>Studio, thoughtfully made.</footer></body></html>`;
...
test('full agent builds real React, detects a deliberate mismatch, repairs it and verifies every viewport',{skip:process.env.MOLT_RUN_FULL_AGENT_TESTS!=='1'},()=>fixture(async dir=>{
  ...
  const model:Model={usage:{calls:0,inputTokens:0,outputTokens:0},async complete(request){
    ...
    return {summary:'Build fixture page with a deliberately incorrect desktop heading',files:[{path:file,content:`export default function Page(){return <><header><img src="${asset}" alt="Studio mark"/><a href="/">Studio</a><a href="/about">About</a></header><main><h1>${title(route)}</h1><p>${text(route)}</p><a className="accent" href="/about">Explore our work</a></main><footer>Studio, thoughtfully made.</footer></>}`},{path:'src/site.css',content:CSS.replace('font-size:48px','font-size:24px')}]};
  }};
