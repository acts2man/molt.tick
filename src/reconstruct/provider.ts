...
Preserve exact visible copy, reading order, imagery, typography, whitespace, crop and section geometry.\nTreat computed font-size, line-height, weight, alignment and measured spacing as numeric constraints, not design suggestions. Do not shrink type or compensate with extra whitespace.\nImage identity is positional: when evidence assigns a localized asset to a hero, portrait, testimonial, gallery cell or background slot, use that exact asset in that slot. Never shuffle, substitute or duplicate a different image because it looks plausible.\nCarousel and slideshow inventories are content contracts: preserve the captured unique slide count, order, text and image mapping, including slides not initially visible. Never collapse a source slideshow into a single static image.\nDo not collapse multiple observed font families into one global font; use the captured font-face evidence and computed font family per region.\nWhen interaction evidence includes carousel/slider Previous or Next states or hover-only navigation menus, reproduce that observed state change at the measured viewport instead of rendering a static approximation.
